...
export class PermissionFakeBuilder<TBuild = any> {
  ...
  static aPermission() {
    return new PermissionFakeBuilder<Permission>();
  }

  static thePermissions(countObjs: number) {
    return new PermissionFakeBuilder<Permission[]>(countObjs);
  }
  ...
  private getValue(prop) {
    ...
    if (!this[privateProp] && optional.includes(prop)) {
      throw new Error(
        `Permission ${prop} not have a factory, use 'with' methods`,
      );
    }
