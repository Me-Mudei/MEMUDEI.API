...
export class PrivacyTypeFakeBuilder<TBuild = any> {
  ...
  static aPrivacyType() {
    return new PrivacyTypeFakeBuilder<PrivacyType>();
  }

  static thePrivacyTypes(countObjs: number) {
    return new PrivacyTypeFakeBuilder<PrivacyType[]>(countObjs);
  }
  ...
  private getValue(prop) {
    ...
    if (!this[privateProp] && optional.includes(prop)) {
      throw new Error(
        `PrivacyType ${prop} not have a factory, use 'with' methods`,
      );
    }
